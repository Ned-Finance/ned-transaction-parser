...
const parseSwap = async (parsed: ParsedInstruction<Idl, string>, connection: Connection): Promise<Partial<ReadableParsedInstruction>> => {

    const args = parsed.args as any
    const from = _.find(parsed.accounts, (account: ParsedAccount) => account.name == 'sourceTokenAccount')!.pubkey.toBase58()
    const to = _.find(parsed.accounts, (account: ParsedAccount) => account.name == 'destinationTokenAccount')!.pubkey.toBase58()
    const authority = _.find(parsed.accounts, (account: ParsedAccount) => account.name == 'userTransferAuthority')!.pubkey.toBase58()
    return {
        data: {
            from,
            to,
