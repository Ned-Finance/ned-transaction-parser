...
const jupiterTransactionV4 = async (props: InferenceFnProps): Promise<InferenceResult> => {
    ...
    if (swap.length == 1) {
        ...
        const transfers = instructions.filter(i => {
            ...
            const isSOLTransfer = i.type == 'SOL_TRANSFER' && (i.data as Transfer).from == walletAddress
            return isSPLTransfer || isSOLTransfer
        })

        console.log('transfers', transfers)

        const ownerTransferIdx = _.findIndex(transfers.filter(i => {
            return (i.data as Transfer).owner == walletAddress || (i.data as Transfer).from == walletAddress
        }))
        ...
        const fromOwnerTransferData = fromOwnerInstruction ? fromOwnerInstruction.data as Transfer : null
        const toOwnerTransferData = toOwnerTransferInstruction ? toOwnerTransferInstruction.data as Transfer : null

        const accountFrom =
            fromOwnerTransferData ? (await getAccountMint(fromOwnerTransferData.from, walletAddress || "", connection)) : ''

        const accountTo =
            toOwnerTransferData ? await getAccountMint(toOwnerTransferData.to, walletAddress || "", connection) : ''
