...
const parseSwapRaydium = async (
	...
	const from = _.find(
		parsed.accounts,
		(account: ParsedAccount) => account.name == "inputTokenAccount"
	)!.pubkey.toBase58();
	const to = _.find(
		parsed.accounts,
		(account: ParsedAccount) => account.name == "outputTokenAccount"
	)!.pubkey.toBase58();
	const slippage = 1 + parseInt(args.inAmountWithSlippage.slippageBps) / 10000;
	const amountIn =
		Number(args.inAmountWithSlippage.amount) * (slippage > 0 ? slippage : 1);
	return {
		data: {
			from,
			to,
...
const route = async (
	...
	const from =
		_.find(
			parsed.accounts,
			(account: ParsedAccount) => account.name == "Remaining 15"
		)?.pubkey.toBase58() ?? "";
	const to =
		_.find(
			parsed.accounts,
...
export default async (
	...
	const [type, partialTransaction]: HumanizeMatchResult = await match(
		...
		.with(
			...
			async () =>
				new Promise<HumanizeMatchResult>(async (resolve) =>
					resolve(["JUPITER_SWAP_V4", await route(parsed, connection)])
				)
		)
		.otherwise(
			async () =>
