...
import { InferenceFnProps, InferenceResult, Transfer } from "../types";

const splTransferMultiple = async (
	props: InferenceFnProps
): Promise<InferenceResult> => {
	const { instructions, tokens, walletAddress, connection } = props;
	const transferInstruction = instructions
		.filter((i) => i.type == "SPL_TRANSFER")
		.map((i) => i.data) as Transfer[];
		...
		return new Promise(async (resolve) => {
			...
			if (tokenObject) {
				const getAction = () => {
					if (walletAddress) {
						const ata = getAssociatedTokenAddressSync(
							new PublicKey(tokenObject.address),
				...
				const accountFromPromise = getAccount(connection, new PublicKey(from))
					...
					.catch(async (e) => {
						if (walletAddress) {
							const isWSOL = await getAssociatedTokenAddressSync(
				...
				if (accountFrom || accountTo) {
					const amount = tokenObject
						? instruction.amount / Math.pow(10, tokenObject.decimals)
						: instruction.amount;
