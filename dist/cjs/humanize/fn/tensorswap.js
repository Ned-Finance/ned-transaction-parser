...
const parseList = async (parsed, connection) => {
	const mintAccount = parsed.accounts.find(
		(account) => account.name == "nftMint"
	);
	// console.log('parsed.accounts', parsed.accounts)
	// console.log('mintAccount', mintAccount)
	const getMetadata = async () => {
		if (mintAccount) {
			const metadata = await (0, nft_1.getMetadataFromAddress)(
				connection,
				mintAccount.pubkey.toBase58()
			);
			const metadataFromUri = await (0, nft_1.getNftMetadataFromUri)(
				metadata.data.uri
			);
	...
	return {
		data: {
			...
			marketplace: "TENSOR",
			nftMint:
				mintAccount === null || mintAccount === void 0
					? void 0
					: mintAccount.pubkey.toBase58(),
			name: metadata.name,
...
exports.default = async (parsed, connection) => {
	const getType = () =>
		(0, ts_pattern_1.match)(parsed.name)
			.with("list", () => "LIST_NFT")
