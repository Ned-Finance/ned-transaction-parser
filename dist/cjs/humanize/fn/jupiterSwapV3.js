...
exports.default = async (parsed, connection) => {
    const [type, partialTransaction] = await (0, ts_pattern_1.match)(parsed.name)
        .with('whirlpoolSwap', async () => new Promise(async (resolve) => resolve(['JUPITER_SWAP_V3', (await parseOrca(parsed, connection))])))
        .with('raydiumSwapV2', async () => new Promise(async (resolve) => resolve(['JUPITER_SWAP_V3', (await parseSwapRaydium(parsed, connection))])))
        .otherwise(async () => new Promise(async (resolve) => resolve(['UNKNOWN', (await defaultHandler(parsed))])));
    // console.log('Jupiter Program V3:', partialTransaction)
    return {
...
//# sourceMappingURL=jupiterSwapV3.js.map