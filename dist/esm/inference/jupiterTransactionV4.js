...
const jupiterTransactionV4 = async (props) => {
    ...
    if (swap.length == 1) {
        ...
        const accountFrom = fromOwnerTransferData ? (await getAccountMint(fromOwnerTransferData.from, walletAddress || "", connection)) : '';
        const accountTo = toOwnerTransferData ? await getAccountMint(toOwnerTransferData.to, walletAddress || "", connection) : '';
